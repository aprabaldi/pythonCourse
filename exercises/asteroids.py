...
def group_group_collide(a_set, other_set):
    result = 0
    for item in list(a_set):
        if group_collide(other_set,item):
            result += 1
            a_set.discard(item)
    return result
...
def rock_spawner():
    ...
    posneg1 = random.choice([-1,1])
    posneg2 = random.choice([-1,1])
    posneg3 = random.choice([-1,1])
    if started:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        vel_multiplier = (score * 2) * 0.1
        rock_vel = [(random.random() * vel_multiplier - .3) * posneg1, (random.random() * vel_multiplier - .3) * posneg2]
        rock_avel = (random.random() * .2 - .1) * posneg3
        
        if len(rock_group) < 12 and dist(my_ship.get_position(), rock_pos) > 200:
